...
info:
  ...
  version: 1.0.0
paths:
  /tournament/{tournament}/process_check_ins.json:
    post:
      summary: Process check-in results for a tournament
      deprecated: true
      description: >-
        This should be invoked after a tournament's check-in window closes
        before the tournament is started.


        1. Marks participants who have not checked in as inactive.

        2. Moves inactive participants to bottom seeds (ordered by original
        seed).

        3. Transitions the tournament state from 'checking_in' to 'checked_in'


        NOTE: Checked in participants on the waiting list will be promoted if
      ...
      parameters:
        ...
        - name: include_participants
          in: query
          description: 0 or 1; includes an array of associated participant records
          required: false
          schema:
            type: string
        - name: include_matches
          in: query
          description: 0 or 1; includes an array of associated match records
          required: false
          schema:
            type: string
      responses:
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/1113893/apis/api-23824997-run
components:
  schemas: {}
  securitySchemes:
    api_key:
