...
paths:
  /tournaments/{tournament}/abort_check_in.json:
    post:
      summary: Abort check-in for a tournament
      deprecated: true
      description: >-
        When your tournament is in a 'checking_in' or 'checked_in' state,
        there's no way to edit the tournament's start time (start_at) or
        check-in duration (check_in_duration). You must first abort check-in,
        then you may edit those attributes.


        Makes all participants active and clears their checked_in_at times.

        Transitions the tournament state from 'checking_in' or 'checked_in' to
        'pending'
      tags:
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/1113893/apis/api-23825012-run
components:
  schemas: {}
  securitySchemes:
    api_key:
